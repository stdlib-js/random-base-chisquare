...
tape( 'the function throws an error if degrees of freedom `k` is not a positive number (options)', function test( t ) {
	var values;
	var i;
...
tape( 'the function returns a seeded pseudorandom number generator (integer seed)', function test( t ) {
	...
	chisquare2 = factory( 35, {
		'seed': seed
	});

	t.notEqual( chisquare1, chisquare2, 'separate generators' );
...
tape( 'attached to the returned function is the generator name', function test( t ) {
	...
	t.strictEqual( chisquare.NAME, 'chisquare', 'has property' );
	t.end();
});

tape( 'attached to the returned function is the underlying PRNG', function test( t ) {
	var chisquare = factory();
	t.strictEqual( typeof chisquare.PRNG, 'function', 'has property' );

	chisquare = factory({
		'prng': minstd.normalized
	});
	t.strictEqual( chisquare.PRNG, minstd.normalized, 'has property' );
	t.end();
});
...
tape( 'attached to the returned function is the generator seed (integer seed)', function test( t ) {
	...
	t.strictEqual( isUint32Array( chisquare.seed ), true, 'has property' );
	t.strictEqual( chisquare.seed[ 0 ], 12345, 'equal to provided seed' );

	chisquare = factory({
		'seed': 12345,
		'prng': minstd.normalized
	});
	t.strictEqual( chisquare.seed, null, 'equal to `null`' );
	t.end();
});
...
tape( 'attached to the returned function is the generator seed length', function test( t ) {
	var chisquare = factory();
	t.strictEqual( typeof chisquare.seedLength, 'number', 'has property' );

	chisquare = factory({
		'prng': minstd.normalized
	});
	t.strictEqual( chisquare.seedLength, null, 'equal to `null`' );
	t.end();
});

tape( 'attached to the returned function is the generator state', function test( t ) {
	var chisquare = factory();
	t.strictEqual( isUint32Array( chisquare.state ), true, 'has property' );

	chisquare = factory({
		'prng': minstd.normalized
	});
	t.strictEqual( chisquare.state, null, 'equal to `null`' );
	t.end();
});

tape( 'attached to the returned function is the generator state length', function test( t ) {
	var chisquare = factory();
	t.strictEqual( typeof chisquare.stateLength, 'number', 'has property' );

	chisquare = factory({
		'prng': minstd.normalized
	});
	t.strictEqual( chisquare.stateLength, null, 'equal to `null`' );
	t.end();
});

tape( 'attached to the returned function is the generator state size', function test( t ) {
	var chisquare = factory();
	t.strictEqual( typeof chisquare.byteLength, 'number', 'has property' );

	chisquare = factory({
		'prng': minstd.normalized
	});
	t.strictEqual( chisquare.byteLength, null, 'equal to `null`' );
	t.end();
});
...
tape( 'attached to the returned function is a method to serialize the generator as a JSON object', function test( t ) {
	...
	chisquare = factory();
	t.strictEqual( typeof chisquare.toJSON, 'function', 'has method' );

	o = chisquare.toJSON();
	t.strictEqual( o.type, 'PRNG', 'has property' );
	t.strictEqual( o.name, chisquare.NAME, 'has property' );
	t.deepEqual( o.state, typedarray2json( chisquare.state ), 'has property' );
	t.deepEqual( o.params, [], 'has property' );

	chisquare = factory( 4 );
	o = chisquare.toJSON();

	t.deepEqual( o.params, [ 4 ], 'has property' );

	chisquare = factory({
		'prng': minstd.normalized
	});
	t.strictEqual( typeof chisquare.toJSON, 'function', 'has method' );
	t.strictEqual( chisquare.toJSON(), null, 'returns expected value' );
...
tape( 'when called without arguments, the function returns a PRNG that returns `NaN` when provided a first argument which is not a positive number', function test( t ) {
	var chisquare;
	var r;
...
tape( 'the function returns a PRNG for generating random numbers from a chi-square distribution', opts, function test( t ) {
	var threshold;
	var count;
	var npass;
	var N;
	...
	function gof() {
		...
		for ( i = 0; i < N; i++ ) {
			chisquare = factory( k );
			t.ok( true, 'seed: '+chisquare.seed );
			for ( j = 0; j < x.length; j++ ) {
				x[ j ] = chisquare();
			}
			// Test using Kolmogorov-Smirnov goodness-of-fit test:
			pValue = kstest( x, 'chisquare', k ).pValue;
			t.strictEqual( typeof pValue, 'number', 'returns a p-value: '+pValue );
			if ( pValue < 0.05 ) {
				rejected += 1;
			}
		}
	...
	function done( bool, rejected ) {
		t.ok( bool, 'null hypothesis (i.e., that numbers are drawn from Chisquare('+k+') is rejected in less than '+(threshold*100)+'% of cases ('+rejected+' of '+N+'). Repeats: '+npass+' of '+count+'.' );
		t.end();
	}
});
...
tape( 'the function supports specifying the generator state', function test( t ) {
	...
	for ( i = 0; i < 100; i++ ) {
		t.strictEqual( chisquare(), arr[ i ], 'returns expected value. i: '+i+'.' );
	}
	t.end();
});
